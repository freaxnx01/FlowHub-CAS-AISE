...
**Semantic Search:** pgvector `vector(384)` column on `Captures` with HNSW index. `AiEmbeddingService` wraps any OpenAI-compatible embedder via MEAI (`mistral-embed`, or a self-hosted `multilingual-e5-small`); `IEmbeddingService` port in `FlowHub.Core`, graceful no-op (search returns 503) when unconfigured. Surface: `GET /api/v1/captures/search?q=...`; admin rebuild `POST /api/v1/admin/embeddings/rebuild`. Disabled on the public demo (503, transparent); covered by integration tests. A self-hosted embedder + a web search UI were trialled on the demo and pulled — modest ranking on a tiny model wasn't worth the VPS memory.
...
**The Obsidian vault as a 2nd brain.** A recurring force-multiplier across all blocks (not just Block 5) was treating the `vault/` (plain Markdown) as a shared **second brain** the agent both **reads** and **writes**: Claude Code pulls CAS scope, block content and prior decisions from it as context, and writes back Nachbereitungen, knowledge notes and the consolidated Learnings. Markdown is the key — human-readable *and* losslessly LLM-consumable, git-diffable, no export/API layer in between. The `vault/CLAUDE.md` conventions (`claude-generated`/`claude-updated` tags, folder ownership, auto-commit) keep human and agent writes auditable. Net effect: what the human notes in a lecture becomes agent context, and what the agent works out becomes a searchable note in the same store.
