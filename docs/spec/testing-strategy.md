...
| FlowHub.Persistence | 100.0 % | 97.2 % | **Testcontainers run is what makes this measurable** — the unit-only number is ~9 %. Union with `FlowHub.Api.IntegrationTests` (InMemory) covers the `FlowHubDbContext.OnModelCreating` non-Npgsql arm |
| FlowHub.Web | 100.0 % | 93.7 % | `[ExcludeFromCodeCoverage]` on `E2EFaultExtensions` + `FlowHubDbContextFactory` |
