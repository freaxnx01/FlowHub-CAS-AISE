...
- **Quarkus-/Jakarta-EE-Kriterium (max. 10 Pkt.):** Die **freie Stackwahl wurde in der PVA explizit bestätigt**; der Moodle-Auftrag nennt Quarkus / Jakarta EE nur als Referenz-Stack, die Lernziele sind stack-neutral. Dieses Kriterium wird daher **über die .NET-Äquivalente erfüllt** (nicht ausgeklammert): die adressierten **Konzepte moderner Enterprise-Applikationen** — DI, deklaratives REST, ORM-Abstraktion, Config, Health/Metrics, async & Messaging, Fehlertoleranz, schlanke Container, Testbarkeit — sind vollständig umgesetzt und im Code nachgewiesen (Konzept-Mapping in [`docs/spec/modern-app-concepts.md`](https://github.com/freaxnx01/FlowHub-CAS-AISE/blob/main/docs/spec/modern-app-concepts.md)). Angestrebte Stufe **„überwiegend"–„vollständig" (7–10)**. Rein JVM-/Quarkus-runtime-spezifische Mechanismen (CDI-Annotationen, GraalVM-Native-Image) werden ehrlich nicht behauptet. Selbst bei strikt JVM-gebundener Auslegung bleiben die übrigen 90 Punkte unberührt (konservative Untergrenze).
